...
// 公共state modles 名称
const APP_COMMON_SPACE_NAME = 'publicModle';
/**
 * 生产state
...
const getMapState = (pageName, modleName) => {
  return state => {
    ...
    const re =
      models[APP_COMMON_SPACE_NAME] &&
      models[APP_COMMON_SPACE_NAME].state
        ? genState(
            Object.keys(
              models[APP_COMMON_SPACE_NAME].state
            ),
...
const getMapDispatch = modleName => {
  return dispatch => {
    ...
    // 首先导入app 里的action
    models[APP_COMMON_SPACE_NAME] &&
      models[APP_COMMON_SPACE_NAME].effects &&
      Object.keys(
        models[APP_COMMON_SPACE_NAME].effects
      ).map(function(action) {
        re[action] = genAction(
    ...
    if (modleName && modleName !== '') {
      Object.keys(models[modleName].effects).map(function(
        action
      ) {
